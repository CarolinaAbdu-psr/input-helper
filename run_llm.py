...
def generate_query(state: GraphState, llm: BaseChatOpenAI) -> Dict[str, Any]:
    ...
    cypher_query = parse_multiple_queries(response)
    
    logger.info(f"Query Cypher generated: {cypher_query[:200]}...")
    print(cypher_query)
    return {"cypher_query": cypher_query}

def parse_multiple_queries(response: str) -> List[str]:
...
def execute_query(state: GraphState) -> Dict[str, Any]:
    ...
    results = "Each query got teh following result: \n "
    ...
    for cypher_query in queries:
        ...
        results += f"{cypher_query}: {query_result_str} \n"

        logger.info(f"Query {cypher_query} Result: {query_result_str[:100]}...")
